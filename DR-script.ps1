...
## Description: Promote VM A-J to the DR site 
## Language: PowerShell
##
############################################################
...
if (!$nxIPs) {
	# Gather Nutanix Cluster address DR Site
	$nxIPs = ((Read-Host "Please enter the target Nutanix Cluster address (DR Site)")) 
}

# Connect to Nutanix Cluster
$ntnxConn = NTNX-Connect-NTNX -ip $_ -credential `
